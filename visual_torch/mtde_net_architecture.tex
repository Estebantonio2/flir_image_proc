...
\begin{tikzpicture}[
    ...
    % --- Rama Tabular (Abajo) ---
    \node [input, below=of img, yshift=-1.0cm] (tab) {Metadatos\\ Tabulares\\ \small(4 variables)};
...
\begin{tikzpicture}[
    ...
    \draw [->, thick] (in.east) -- +(0.3,0) -- +(0.3,-2.2) -| node[pos=0.4, below, font=\scriptsize, align=center] {Conv 1x1 + BN\\ (si $C_{in} \neq C_{out}$)} (sum_skip.south);
